...
import { Menu, X, Home, LogIn, LogOut, User, Building2, Bookmark } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-6">
            {mainNavLinks.map((link) => (
              <Link key={link.to} to={link.to} className={linkClass(false)}>
                {link.label}
              </Link>
            ))}
            ...
            <Link to={brokersLink.to} className={linkClass(brokersLink.highlight)}>
              {brokersLink.label}
            </Link>
