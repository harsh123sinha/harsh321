...
const BrokerReviewModal = ({ open, onClose, brokerId, brokerName, brokerPhoto, propertyId, notificationId }) => {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/broker-customer-reviews', {
        brokerId,
        propertyId,
        rating,
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <div className="flex items-center gap-4 mb-4 pr-8">
          <div className="h-14 w-14 rounded-full bg-navy text-gold flex items-center justify-center font-bold overflow-hidden flex-shrink-0">
            {brokerPhoto ? (
              <img src={brokerPhoto} alt="" className="h-full w-full object-cover" />
            ) : (
              brokerInitials(brokerName)
            )}
          </div>
          <div>
            <h2 className="text-lg font-bold text-navy">Rate your broker</h2>
            <p className="text-sm text-gray">
              How was your experience with <strong className="text-navy">{brokerName}</strong>?
            </p>
          </div>
        </div>
...
export default BrokerReviewModal;
