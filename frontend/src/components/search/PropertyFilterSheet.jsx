...
const PropertyFilterSheet = ({ presetLocation = '', presetType = '' }) => {
  ...
  return createPortal(
    <div
      className="htls-filter-sheet fixed inset-0 z-[52] flex flex-col bg-white lg:hidden"
      role="dialog"
      aria-modal="true"
      aria-label="Property filters"
    >
      <header className="flex shrink-0 items-center gap-2 border-b border-navy/10 bg-navy px-2 py-2.5 pt-[max(0.5rem,env(safe-area-inset-top))] shadow-md">
        <button
          type="button"
          onClick={closeFilter}
          aria-label="Close filters"
          className="flex h-10 w-10 items-center justify-center rounded-full text-white transition hover:bg-white/10 touch-manipulation"
        >
          <ChevronLeft className="h-6 w-6" />
        </button>
        <h2 className="text-base font-bold tracking-wide text-white">Filters</h2>
      </header>
